...
(function () {
    ...
    var sendToSocket = function () {
        var lazyBotSettings = lazyBot.settings;
        var lazyBotRoom = lazyBot.room;
        var lazyBotInfo = {
            time: Date.now(),
            version: lazyBot.version
        };
        var data = {users:API.getUsers(),userinfo:API.getUser(),room:location.pathname,lazyBotSettings:lazyBotSettings,lazyBotRoom:lazyBotRoom,lazyBotInfo:lazyBotInfo};
    ...
    var lazyBot = {
        ...
        settings: {
            ...
            commandLiteral: "!",
            blacklists: {
                NSFW: "https://rawgit.com/lazyBot/custom/master/blacklists/NSFWlist.json",
                OP: "https://rawgit.com/lazyBot/custom/master/blacklists/OPlist.json",
                BANNED: "https://rawgit.com/lazyBot/custom/master/blacklists/BANNEDlist.json"
            }
        },
        room: {
            name: null,
        ...
        userUtilities: {
            ...
            dclookup: function (id) {
                ...
                if (newPosition <= 0) return subChat(lazyBot.chat.notdisconnected, {name: name});
        ...
        roomUtilities: {
            ...
            smartSkip: function (reason) {
                ...
                setTimeout(function (id) {
                    ...
                    setTimeout(function (id) {
                        lazyBot.userUtilities.moveUser(id, lazyBot.settings.skipPosition, false);
            ...
            changeDJCycle: function () {
                $.getJSON('/_/rooms/state', function(data) {
                    if (data.data[0].booth.shouldCycle) { // checks "" "shouldCycle": true "" if its true
                        ...
                        clearTimeout(lazyBot.room.cycleTimer); // Clear the cycleguard timer
                    } else { // If cycle is already disable; enable it
                        if (lazyBot.settings.cycleGuard) { // Is cycle guard on?
                        API.moderateDJCycle(true); // Enables DJ cycle
                        lazyBot.room.cycleTimer = setTimeout(function () {  // Start timer
                            API.moderateDJCycle(false); // Disable cycle
                        }, lazyBot.settings.maximumCycletime * 60 * 1000); // The time
        ...
        eventChat: function (chat) {
            ...
            lazyBot.room.chatMessages.push([chat.cid, chat.message, chat.sub, chat.timestamp, chat.type, chat.uid, chat.un]);
        ...
        eventUserleave: function (user) {
            ...
            for (var i = 0; i < lazyBot.room.users.length; i++) {
                if (lazyBot.room.users[i].id === user.id) {
                    ...
                    if (lastDJ == user.id){
                        var user = lazyBot.userUtilities.lookupUser(lazyBot.room.users[i].id);
                        lazyBot.userUtilities.updatePosition(user, 0);
        ...
        eventVoteupdate: function (obj) {
            ...
            if (lazyBot.settings.voteSkip) {
                if ((mehs - woots) >= (lazyBot.settings.voteSkipLimit)) {
                    ...
                    if (lazyBot.settings.smartSkip && timeLeft > timeElapsed){
                        lazyBot.roomUtilities.smartSkip();
                    }
                    else {
                        API.moderateForceSkip();
                    }
                ...
                for (var bl in lazyBot.room.blacklists) {
                    if (lazyBot.settings.blacklistEnabled) {
                        if (lazyBot.room.blacklists[bl].indexOf(mid) > -1) {
                            ...
                            if (lazyBot.settings.smartSkip){
                                return lazyBot.roomUtilities.smartSkip();
                            }
                            else {
                ...
                if (format == 1){
                    $.getJSON('https://www.googleapis.com/youtube/v3/videos?id=' + cid + '&key=AIzaSyDcfWu9cGaDnTjPKhg_dy9mUh6H7i4ePZ0&part=snippet&callback=?', function (track){
                        if (typeof(track.items[0]) === 'undefined'){
                            var name = obj.dj.username;
                            API.sendChat(subChat(lazyBot.chat.notavailable, {name: name}));
                            if (lazyBot.settings.smartSkip){
                                return lazyBot.roomUtilities.smartSkip();
                            }
                            else {
        ...
        chatUtilities: {
            chatFilter: function (chat) {
                ...
                if (lazyBot.settings.cmdDeletion && msg.startsWith(lazyBot.settings.commandLiteral)) {
                    API.moderateDeleteChat(chat.cid);
                }
                /**
            ...
            commandCheck: function (chat) {
                var cmd;
                if (chat.message.charAt(0) === lazyBot.settings.commandLiteral) {
                ...
                if (chat.message !== lazyBot.settings.commandLiteral + 'join' && chat.message !== lazyBot.settings.commandLiteral + "leave") {
                    if (userPerm === 0 && !lazyBot.room.usercommand) return void (0);
                    if (!lazyBot.room.allcommand) return void (0);
                }
                if (chat.message === lazyBot.settings.commandLiteral + 'eta' && lazyBot.settings.etaRestriction) {
                    if (userPerm < 2) {
        ...
        startup: function () {
            ...
            lazyBot.room.name = window.location.pathname;
            var Check;

            console.log(lazyBot.room.name);

            var detect = function(){
                if(lazyBot.room.name != window.location.pathname){
                    ...
                    if (lazyBot.settings.roomLock){
                        window.location = lazyBot.room.name;
                    }
                    else {
                        clearInterval(Check);
                    }
        ...
        commands: {
            ...
            autoskipCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!lazyBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (lazyBot.settings.autoskip) {
                            lazyBot.settings.autoskip = !lazyBot.settings.autoskip;
                            clearTimeout(lazyBot.room.autoskipTimer);
            ...
            ballCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!lazyBot.commands.executable(this.rank, chat)) return void (0);
                            ...
                            var randomBall = Math.floor(Math.random() * lazyBot.chat.balls.length);
                            var randomSentence = Math.floor(Math.random() * 1);
                            API.sendChat(subChat(lazyBot.chat.ball, {name: chat.un, botname: lazyBot.settings.botName, question: argument, response: lazyBot.chat.balls[randomBall]}));
                     }
                }
            },
            ...
            botnameCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!lazyBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        var msg = chat.message;
                        if (msg.length <= cmd.length + 1) return API.sendChat(subChat(lazyBot.chat.currentbotname, {botname: lazyBot.settings.botName}));
                        var argument = msg.substring(cmd.length + 1);
                        if (argument) {
                            lazyBot.settings.botName = argument;
                            API.sendChat(subChat(lazyBot.chat.botnameset, {botName: lazyBot.settings.botName}));
                        }
                    }
                }
            },
            ...
            cookieCommand: {
                ...
                getCookie: function (chat) {
                    var c = Math.floor(Math.random() * lazyBot.chat.cookies.length);
                    return lazyBot.chat.cookies[c];
                },
                functionality: function (chat, cmd) {
                    if (this.type === 'exact' && chat.message.length !== cmd.length) return void (0);
                    if (!lazyBot.commands.executable(this.rank, chat)) return void (0);
            ...
            deletechatCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!lazyBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        var user = lazyBot.userUtilities.lookupUserName(name);
                        if (typeof user === 'boolean') return API.sendChat(subChat(lazyBot.chat.invaliduserspecified, {name: chat.un}));
                        for (var i = 1; i < lazyBot.room.chatMessages.length; i++) {
                          if (lazyBot.room.chatMessages[i].indexOf(user.id) > -1){
                            API.moderateDeleteChat(lazyBot.room.chatMessages[i][0]);
                            lazyBot.room.chatMessages[i].splice(0);
                          }
                        }
                        API.sendChat(subChat(lazyBot.chat.deletechat, {name: chat.un, username: name}));
                    }
            ...
            etaCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!lazyBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        var realpos = pos + 1;
                        if (name == dj) return API.sendChat(subChat(lazyBot.chat.youaredj, {name: name}));
                        if (pos < 0) return API.sendChat(subChat(lazyBot.chat.notinwaitlist, {name: name}));
                        if (pos == 0) return API.sendChat(subChat(lazyBot.chat.youarenext, {name: name}));
                        var timeRemaining = API.getTimeRemaining();
                        var estimateMS = ((pos + 1) * 4 * 60 + timeRemaining) * 1000;
                        var estimateString = lazyBot.roomUtilities.msToStr(estimateMS);
                        API.sendChat(subChat(lazyBot.chat.eta, {name: name, time: estimateString, position: realpos}));
                    }
                }
            },
            ...
            forceskipCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!lazyBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        API.sendChat(subChat(lazyBot.chat.forceskip, {name: chat.un}));
                        API.moderateForceSkip();
                        lazyBot.room.skippable = false;
                        setTimeout(function () {
                            lazyBot.room.skippable = true
            ...
            languageCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!lazyBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        $.get("https://rawgit.com/lazyBot/source/master/lang/langIndex.json", function (json) {
                            var langIndex = json;
            ...
            reloadCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!lazyBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        kill();
                        setTimeout(function () {
                            $.getScript(lazyBot.settings.scriptLink);
                        }, 2000);
                    }
                }
            },
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!lazyBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (lazyBot.room.skippable) {
                            ...
                            if (chat.message.length === cmd.length) {
                                API.sendChat(subChat(lazyBot.chat.usedskip, {name: chat.un}));
                                if (lazyBot.settings.smartSkip && timeLeft > timeElapsed){
                                    lazyBot.roomUtilities.smartSkip();
                                }
                                else {
                            ...
                            var msg = chat.message;
                            var reason = msg.substring(cmd.length + 1);
                            for (var i = 0; i < lazyBot.settings.skipReasons.length; i++) {
                                var r = lazyBot.settings.skipReasons[i][0];
                                if (reason.indexOf(r) !== -1) {
                                    validReason = true;
                                    msgSend += lazyBot.settings.skipReasons[i][1];
                                }
                            }
                            if (validReason) {
                                API.sendChat(subChat(lazyBot.chat.usedskip, {name: chat.un}));
                                if (lazyBot.settings.smartSkip && timeLeft > timeElapsed){
                                    lazyBot.roomUtilities.smartSkip(msgSend);
                                }
                                else {
                                    API.moderateForceSkip();
                                    setTimeout(function () {
            ...
            skipposCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!lazyBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (!isNaN(pos)) {
                            lazyBot.settings.skipPosition = pos;
                            return API.sendChat(subChat(lazyBot.chat.skippos, {name: chat.un, position: lazyBot.settings.skipPosition}));
                        }
                        else return API.sendChat(subChat(lazyBot.chat.invalidpositionspecified, {name: chat.un}));
                    }
                }
            ...
            statusCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!lazyBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (lazyBot.settings.autoskip) msg += 'ON';
                        else msg += 'OFF';
                        msg += '. ';
                    ...
                    if (!lazyBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (p1 === -1){
                            ...
                            setTimeout(function (user1, p2) {
                                lazyBot.userUtilities.moveUser(user1.id, p2, true);
                            }, 2000, user1, p2);
                        }
                        else if (p2 === -1){
                            API.moderateRemoveDJ(user1.id);
                            setTimeout(function (user2, p1) {
                                lazyBot.userUtilities.moveUser(user2.id, p1, true);
                            }, 2000, user2, p1);
                        }
                        else if (p1 < p2) {
            ...
            thorCommand: {
              ...
              functionality: function (chat, cmd) {
                    ...
                    if (!lazyBot.commands.executable(this.rank, chat)) return void (0);
                      ...
                      if (lazyBot.settings.thorCommand){
                        var id = chat.uid,
                          ...
                          if (inDjList) {
                              for (var i = 0; i < lazyBot.room.usersUsedThor.length; i++) {
                                  if (lazyBot.room.usersUsedThor[i].id == id) {
                                      oldTime = lazyBot.room.usersUsedThor[i].time;
                                      usedThor = true;
                                      indexArrUsedThor = i;
                                  }
                              }
                              ...
                              if (usedThor) {
                                  timeInMinutes = (lazyBot.settings.thorCooldown + 1) - (Math.floor((oldTime - Date.now()) * Math.pow(10, -5)) * -1);
                                  thorCd = timeInMinutes > 0 ? true : false;
                                  if (thorCd == false)
                                      lazyBot.room.usersUsedThor.splice(indexArrUsedThor, 1);
                              }

                              if (thorCd == false || usedThor == false) {
                                  var user = {id: id, time: Date.now()};
                                  lazyBot.room.usersUsedThor.push(user);
                              }
                          }

                          if (!inDjList) {
                              return API.sendChat(subChat(lazyBot.chat.thorNotClose, {name: from}));
                          } else if (thorCd) {
                              return API.sendChat(subChat(lazyBot.chat.thorcd, {name: from, time: timeInMinutes}));
                          }

                          if (worthy) {
                            if (API.getWaitListPosition(id) != 0)
                            lazyBot.userUtilities.moveUser(id, 1, false);
                            API.sendChat(subChat(lazyBot.chat.thorWorthy, {name: from}));
                          } else {
                            if (API.getWaitListPosition(id) != djlist.length - 1)
                            lazyBot.userUtilities.moveUser(id, djlist.length, false);
                            API.sendChat(subChat(lazyBot.chat.thorNotWorthy, {name: from}));
                          }
                        }
                    }
                }
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!lazyBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        $.getJSON('/_/bans', function (json){
                            ...
                            if (!found) return API.sendChat(subChat(lazyBot.chat.notbanned, {name: chat.un}));
                        ...
                        $.getJSON('/_/mutes', function (json){
                            ...
                            if (msg.indexOf('@') === -1 && arg === 'all'){
                                if (permFrom > 2){
                                    ...
                                    API.sendChat(subChat(lazyBot.chat.unmutedeveryone, {name: chat.un}));
                                } else API.sendChat(subChat(lazyBot.chat.unmuteeveryonerank, {name: chat.un}));
                            } else {
                                for (var i = 0; i < mutedUsers.length; i++){
                    ...
                    if (!lazyBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        for (var i = 0; i < len; ++i){
                            if (users[i].username == name){
                                ...
                                API.sendChat(subChat(lazyBot.chat.whois, {name1: chat.un, name2: name, id: id, avatar: avatar, profile: profile, language: language, level: level, joined: joined, rank: rank}));
                            }
                        }
                    }
                }
